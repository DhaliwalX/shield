...
void makeWindowForGL(NativeWindowInternal* wrapper, NativeWindow* glWindow) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeFromBackendRenderTarget(
      grContext.get(), renderTarget, kTopLeft_GrSurfaceOrigin, nullptr,
      &props));
...
class NativeWindowFactory {
 public:
  std::shared_ptr<NativeWindow> createWindow(GlContext* context,
    ...
    std::shared_ptr<NativeWindowInternal> nativeWindow =
        std::make_shared<NativeWindowInternal>();

    nativeWindow->window = window;
    int id = saveWindow(nativeWindow);
    std::shared_ptr<NativeWindow> glWindow =
        std::make_shared<NativeWindow>(context, id, width, height, x, y);
  ...
  static NativeWindowFactory* getInstance() { return factory_.get(); }

  static void destroy() { factory_->destroyWindows(); }
...
std::unique_ptr<NativeWindowFactory> NativeWindowFactory::factory_ =
    std::make_unique<NativeWindowFactory>();

std::shared_ptr<NativeWindow> GlContext::makeWindow(uint16_t width,
                                                    uint16_t height,
                                                    uint16_t x,
                                                    uint16_t y) {
