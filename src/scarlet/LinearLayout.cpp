...
namespace avenger {
    namespace scarlet {
        ...
        LinearLayout::LinearLayout()
            : children_{}, direction_{ Direction::kVertical }
        { }
        ...
        std::unique_ptr<SkCanvas> LinearLayout::allocCanvas() {
            ...
            auto info = SkImageInfo::MakeN32Premul(SkScalarRoundToInt(width), SkScalarRoundToInt(height));
        ...
        void LinearLayout::onCreate(SkCanvas *can) {
            if (!isDirty()) {
                can->drawBitmap(*bitmap_,
                                   getLayoutParams()->getX(),
            ...
            can->drawBitmap(*bitmap_, getLayoutParams()->getX(), getLayoutParams()->getY());
        }
