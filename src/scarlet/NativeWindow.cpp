...
NativeWindow::NativeWindow(GlContext* context,
                           int id,
                           uint16_t width,
                           uint16_t height,
                           uint16_t x,
...
void NativeWindow::correctScaling() {
  getCanvas()->scale((float)width / drawableArea.width,
                     (float)height / drawableArea.height);
}
...
sk_sp<SkSurface> NativeWindow::createOffScreenCanvas() {
  ...
  auto imageInfo = SkImageInfo::Make(
      getHeight(), getWidth(), SkColorType::kBGRA_8888_SkColorType,
