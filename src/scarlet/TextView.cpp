...
namespace avenger {
    namespace scarlet {
        ...
        void TextViewData::setFont(const std::string &fontName) {
            auto typeface = SkTypeface::MakeFromName(fontName.c_str(),
                                                     SkFontStyle::Normal());
        ...
        void TextView::onCreate(SkCanvas *canvas) {
            ...
            canvas->drawText(data_.text_.c_str(),
                             data_.text_.length(),
                             getLayoutParams()->getX(),
                             getLayoutParams()->getY() + dimensions.height(),
                             data_.metadata_->paint);
        }
