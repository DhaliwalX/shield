...
class Task {
 public:
  Task(std::shared_ptr<Event> event,
       std::function<void(std::shared_ptr<Event>)> function)
      : event_{std::move(event)}, handler_{std::move(function)} {}

  Task(const Task& other) = default;

  void execute() { handler_(event_); }
