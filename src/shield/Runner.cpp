...
int avenger::Runner::Run(int argc, char **argv) {
...
//                << (double)averageSwapTiming.count() / sampleCount / 1000.0
//                << "ms (average)" << std::endl;
//
//      // reset counters
//      counter = 0;
...
//                  area.height, canvas->imageInfo().width(),
...
void avenger::Runner::RunPythonThread(int argc, char **argv) {
  thread_ = std::make_unique<std::thread>([&]{
    Runner::Run(argc, argv);
  });
}

std::unique_ptr<std::thread> avenger::Runner::thread_;
